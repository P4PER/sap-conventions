...
### 9.1 Script decides (deterministic)
...
| 10 | `xs-app.json` route ordering and required flags (§8)                      |
| 11 | `mta.yaml` module/resource names derive from `ID` (§8)                    |

### 9.2 Model judges (reported as questions, never auto-fixed)
...
- `references/approuter.md` is the thinnest section; §8 is derived from three
